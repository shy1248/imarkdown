...
- 自由拖动和调整文档元素的顺序；
- 支持标题折叠；
- 支持自动生成文档 TOC，点击 TOC 条目自动跳转至对应的位置；
- 支持输入 `/` 快速插入；
- 手动输入链接和图片时，支持路径自动补全；
- 支持全文检索和跳转；
...
```json
{
    ...
    // 编辑器布好局模式：compact，moderate，loose，默认值：moderate
    "imarkdown.editor.typesetting": "moderate",
    // 是否显示代码块内的行号，默认值：false
    "imarkdown.editor.showLineNumbers": false
}
```