 ...
 *
 * 已验证的关键约束：
 *   - VSCode webview 滚动容器是 body（overflow-y: auto）。
...
// 触发区固定 80px：光标距 toolbar 底部 80px 内向上滚，距视口底部 80px 内向下滚
...
function scrollStep() {
    ...
    if (_lastY >= 0) {
        ...
        const topTrigger = toolbarH + SCROLL_ZONE;   // 低于此值 → 向上滚
        const bottomTrigger = vh - SCROLL_ZONE;       // 高于此值 → 向下滚

        let targetSpeed = 0;
        if (_lastY < topTrigger) {
            const dist = Math.max(0, _lastY - toolbarH);
            targetSpeed = -computeSpeed(dist, SCROLL_ZONE);
        ...
        if (_currentSpeed !== 0) document.body.scrollBy(0, _currentSpeed);

    } else {
        if (Math.abs(_currentSpeed) >= 0.5) {
            _currentSpeed *= (1 - EASE);
            document.body.scrollBy(0, _currentSpeed);
        } else {
            _currentSpeed = 0;
        }
    }
...
/**
 * 初始化拖拽自动滚动，返回清理函数。
 * rAF 循环始终运行，仅 _lastY >= 0（拖拽中）时才 scrollBy，开销极小。
 */
